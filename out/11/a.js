    ...
    for (let monkeyIndex = 0; monkeyIndex < monkeys.length; monkeyIndex++) {
        ...
        while (currentMonkey.items.length > 0) {
            ...
            if (monkeyIndex < monkeys.length) {
                console.log(`Turn #${monkeyIndex} Round ${Math.floor(monkeyIndex / monkeys.length) + 1} Monkey ${currentMonkeyIndex} threw ${newItemWorry} (${currentItem}) Monkey ${throwToMonkeyIndex} got ${newItemWorry} Test ${passed ? "passed" : "failed"}`);
                showMonkeyItems();
            }
        }
    }

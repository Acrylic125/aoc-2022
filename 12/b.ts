...
function countStepsFromStart(startPos: [number, number]): number {
  let open = [
    {
  ...
  while (true) {
    ...
    for (const direction of traversibleDirections) {
      ...
      if (nx === endPos[0] && ny === endPos[1]) {
        ...
        return currentObj.stepsFromStart + 1;
      }
...
const lowest = possibleStarts
  .map((s) => countStepsFromStart(s))
